...
describe('validateIntent', () => {
	it('капитан текущей команды может дать подсказку в фазе clue', () => {
		expect(validateIntent(roster, game(), 'rs', { type: 'GIVE_CLUE', word: 'x', count: 1 })).toBe(
			true,
		)
	})

	it('капитан другой команды не может дать подсказку', () => {
		expect(validateIntent(roster, game(), 'bs', { type: 'GIVE_CLUE', word: 'x', count: 1 })).toBe(
			false,
		)
	})

	it('игрок не может давать подсказку', () => {
		expect(validateIntent(roster, game(), 'ro', { type: 'GIVE_CLUE', word: 'x', count: 1 })).toBe(
			false,
		)
	})
	...
	it('нельзя дать подсказку в фазе guess и открыть карту в фазе clue', () => {
		expect(
			validateIntent(roster, game({ phase: 'guess' }), 'rs', {
				type: 'GIVE_CLUE',
				word: 'x',
				count: 1,
			}),
		...
		expect(
			validateIntent(roster, game({ phase: 'clue' }), 'ro', { type: 'REVEAL_CARD', index: 0 }),
		).toBe(false)
	})
	...
	it('неизвестный отправитель отклоняется', () => {
		expect(validateIntent(roster, game(), 'xxx', { type: 'GIVE_CLUE', word: 'x', count: 1 })).toBe(
			false,
		)
	})
	...
	it('в завершённой игре любые интенты отклоняются', () => {
		...
		expect(validateIntent(roster, over, 'rs', { type: 'GIVE_CLUE', word: 'x', count: 1 })).toBe(
			false,
		)
	})
})
