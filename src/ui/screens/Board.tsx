...
import { CardTile } from '@/ui/components/CardTile.tsx'
import { RippleButton } from '@/ui/components/RippleButton.tsx'
import { ClueBar } from '@/ui/components/ClueBar.tsx'
import { ClueComposer, openComposer } from '@/ui/components/ClueComposer.tsx'
import { PassPhone } from '@/ui/components/PassPhone.tsx'
import { ScoreChip } from '@/ui/components/ScoreChip.tsx'
import { WordToken } from '@/ui/components/WordToken.tsx'
import { PlayerBadge } from '@/ui/components/PlayerBadge.tsx'
import { Screen } from '@/ui/components/Screen.tsx'
import { TopBar } from '@/ui/components/TopBar.tsx'
import { useEndTurnLock, useGameFeedback, useTurnTimer } from '@/ui/hooks.ts'
...
export const Board = reatomComponent(() => {
	...
	const interactive =
		view.phase === 'guess' && (isLocal || (team === myTeamAtom() && myRoleAtom() === 'operative'))
	const canEnd =
	...
	const canCompose =
		!over &&
		view.phase === 'clue' &&
		(isLocal ? viewRole === 'spymaster' : team === myTeamAtom() && myRoleAtom() === 'spymaster')
	...
	const newGame = () =>
		mode === 'host'
			? hostStartGame({ words: selectedWordsAtom() })
			: startGame({ words: selectedWordsAtom() })
	const toMenu = () => {
		...
		>
			...
			<div className="flex gap-2.5">
				<ScoreChip
					...
					active={team === 'red'}
					mirror
					onClick={() => goToStats('red')}
				/>
				<ScoreChip
					team="blue"
					count={sc.blue}
					active={team === 'blue'}
					onClick={() => goToStats('blue')}
				/>
			</div>
			...
			{canCompose ? (
				<div className="flex gap-2.5">
					<RippleButton size="lg" className={cn('flex-1', TEAM_SOLID[team])} onClick={openComposer}>
						<Lightbulb />
						Придумать подсказку
					</RippleButton>
				</div>
