...
function buildTurns(log: LogEntry[]): Turn[] {
	...
	for (const e of log) {
		if (e.kind === 'clue') {
			current = {
				team: e.team,
				clue: { word: e.word, count: e.count },
				reveals: [],
				passed: false,
				startAt: e.at,
			}
...
export const Stats = reatomComponent(() => {
	...
	return (
		<Screen
			...
			onBack={back}
			header={
				<TopBar title={team ? `Статистика · ${TEAM_NOM[team]}` : 'Статистика'} onBack={back} />
			}
		>
			{team ? (
						...
						>
							...
							<div className="flex items-center gap-2">
								...
								<span className="font-heading font-semibold">
									{turn.clue.word}{' '}
									<span className="text-muted-foreground">· {turn.clue.count}</span>
								</span>
