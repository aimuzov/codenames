...
import { HAPTIC, vibrate } from "../audio/haptics.ts";
...
/** Звук и вибрация на открытие карт и завершение партии. */
  ...
  useEffect(() => {
    ...
    if (!isReset) {
      for (const i of newly) {
        ...
        if (hapticsEnabledAtom())
          vibrate(type === "assassin" ? HAPTIC.assassin : HAPTIC.reveal);
      }
    }
    ...
    if (over && !prevOver.current && !isReset) {
      ...
      if (hapticsEnabledAtom()) vibrate(HAPTIC.win);
    }
...
export function useEndTurnLock(view: RedactedState | null): number {
  ...
  useEffect(() => {
    ...
    const remaining = () =>
      Math.ceil(endTurnLockLeft(startedAt, END_TURN_LOCK_SECONDS, Date.now()));
  ...
  useEffect(() => {
    ...
    const onEnd = (e: TouchEvent) => {
      ...
      if (
        dx >= THRESHOLD &&
        dx > Math.abs(dy) &&
        e.timeStamp - startT <= MAX_DURATION
      ) {
        cb.current?.();
      }
    };
